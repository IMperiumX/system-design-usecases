...
![A diagram of a shopping cart  Description automatically generated](image_rsrc9HU.jpg)
...
![A diagram of a payment system  Description automatically generated](image_rsrc9HV.jpg)
...
![A screenshot of a login screen  Description automatically generated](image_rsrc9HW.jpg)
...
![A diagram of a payment service  Description automatically generated](image_rsrc9HX.jpg)
...
6. The token we received in step 4. The PSP’s javascript code uses the token to retrieve detailed information about the payment request from the PSP’s backend. One important piece of information is how much money to collect.

7. Another important piece of information is the redirect URL. This is the web page URL that is called when the payment is complete. When the PSP’s JavaScript finishes the payment, it redirects the browser to the redirect URL. Usually, the redirect URL is an e-commerce web page that shows the status of the checkout. Note that the redirect URL is different from the webhook [11] URL in step 9.

![A screenshot of a mobile payment  Description automatically generated](image_rsrc9HY.jpg)

Figure 5 Hosted payment page by Stripe

1. The user fills in the payment details on the PSP’s web page, such as the credit card number, holder’s name, expiration date, etc, then clicks the pay button. The PSP starts the payment processing.
2. The PSP returns the payment status.
3. The web page is now redirected to the redirect URL. The payment status that is received in step 7 is typically appended to the URL. For example, the full redirect URL could be [12]: https://your-company.com/?tokenID = JIOUIQ123NSF&payResult=X324FSa
4. Asynchronously, the PSP calls the payment service with the payment status via a webhook. The webhook is an URL on the payment system side that was registered with the PSP during the initial setup with the PSP. When the payment system receives payment events through the webhook, it extracts the payment status and updates the payment\_order\_status field in the Payment Order database table.
...
![A diagram of a payment system  Description automatically generated](image_rsrc9HZ.jpg)
...
Because three parties (client, payment service, PSP) are involved in this process, we need to ensure data is eventually consistent among them, even with network failures. Therefore, all communications have to be idempotent and we need reconciliation to ensure data consistency.
...
![A diagram of a diagram  Description automatically generated](image_rsrc9J3.jpg)

Figure 10 Handle failed payments

1. Check whether the failure is retryable.

2. Retryable failures are routed to a retry queue.

3. For non-retryable failures such as invalid input, errors are stored in a database.

4. The payment system consumes events from the retry queue and retries failed payment transactions.

5. If the payment transaction fails again:

6. If the retry count doesn’t exceed the threshold, the event is routed to the retry queue.

7. If the retry count exceeds the threshold, the event is put in the dead letter queue. Those failed events might need to be investigated.
...
![A diagram of payment system  Description automatically generated](image_rsrc9J4.jpg)
...
![A diagram of a post  Description automatically generated](image_rsrc9J5.jpg)
...
Scenario 2: The payment is successfully processed by the PSP, but the response fails to reach our payment system due to network errors. Then the user clicks “pay” again.
...
To maintain data consistency between the internal service and external service (PSP), we usually rely on idempotency and reconciliation. If the external service supports idempotency, we should use the same idempotency key for payment retry operations. Even if an external service supports idempotency API, reconciliation is still needed because we shouldn’t assume the external system is always right.

If data is replicated, replica lag could cause inconsistent data between the primary database and the replicas. There are generally two options to solve this:
...
* Cash payment. Cash payment is very common in India, Brazil, and some other countries. We have to consider cash when we design the system. Uber [28] and Airbnb [29] wrote detailed engineering blogs about how they handled cash-based payment.
